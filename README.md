
### 这个是干嘛的？
> 校验commit message格式规范，以及在git push阶段，对相应js文件进行校验

### 为什么不在commit阶段进行校验？
> git是分布式的概念，本地怎么开发是个人的自由，我不希望在本地开发阶段去破坏大家原有的开发习惯，因此只在push阶段进行代码校验

### git钩子是什么？
>git能在特定的重要动作发生时触发自定义脚本，这些我们称之为`钩子`，如果该钩子以非零值退出，git 将放弃此次提交，钩子都被存储在git目录下的 hooks 子目录中，这些示例的名字都是以 `.sample` 结尾，如果你想启用它们，得先移除这个后缀。

### 校验的工作原理是什么？
> git push时，把当前分支 和 远端关联分支进行对比，拿到有改动过的文件（剔除删除更改操作），然后对这些文件进行校验，校验通过返回0，否则返回1。

### 这种校验方式有缺陷么？
> 有的。若当前本地分支没有与之对应的远程关联分支，则这个时候获取到的文件是空的，因此返回值是0

### 怎么解决这个缺陷？
> 目前想到的方案有两种：
1. 本地创建分支后，立马把空分支与远端分支关联；
2. commit阶段做一个判断（当前本地分支 是否有 与之对应的远程关联分支），如果没有则中断commit。


### 怎么用前端的姿势来实现？
```
npm install --save-dev eslint husky
npm install -g @commitlint/cli @commitlint/config-conventional

// 注解：
// husky 用于设置本地的 git 钩子
// commitlint 用于校验 commit message格式规范
// 之后在项目根目录创建bin文件夹，分别写2个shell脚本，记得赋予可执行权限，形如 chmod 755 ./bin/js-lint
```

**package.json**
```
{
    ...
    "husky": {
        "hooks": {
            "commit-msg": "commitlint -E HUSKY_GIT_PARAMS",
            "pre-commit": "./bin/heck-branch",
            "pre-push": "./bin/js-lint"
        }
    }
}
```

**./bin/heck-branch**
```
#!/bin/bash
currBranch=$(git branch -vv | grep  $(git symbolic-ref --short HEAD) | grep origin)

if test ${#currBranch} -eq $[0]; then
    echo -e "\033[31m \n 🚫  🚫  🚫   当前分支没有关联远程分支，请先把当前分支推送到远程分支！ \n  \033[0m"
    echo -e "\033[32m \n 可使用 git push --set-upstream origin 你当前的分支名 来关联到远程分支\n    \033[0m"
    exit 1;
else
    exit 0;
fi
```

**./bin/js-lint**
```
#!/bin/bash
currBranch=$(git symbolic-ref --short HEAD)
files=$(git diff HEAD  remotes/origin/$currBranch  --stat --name-only --diff-filter=ACMR -- 'src/*.js')

if test ${#files} -gt 0
    then
        clear && echo -e "\033[32m  \n 🔍  🔍  🔍  开始校验js... \033[0m"
        PASS=true

        for file in $files
            do
                ./node_modules/eslint/bin/eslint.js $file --cache
                if [[ "$?" == 1 ]]; then
                    PASS=false
                fi
            done

        if ! $PASS; then
            echo -e "\033[31m \n 🚫  🚫  🚫   eslint校验不通过 \n  \033[0m"
            exit 1
        else
            echo -e "\033[32m \n ✅  ✅  ✅   eslint校验通过 \n    \033[0m"
        fi

    else
        echo -e "\033[32m   \n 🔔  🔔  🔔   没有js文件需要检查 \n   \033[0m"
fi

exit 0
```
